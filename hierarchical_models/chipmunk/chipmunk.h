...
class Chipmunk : public Renderizable {
    ...
    static std::shared_ptr<CompositionNode> build_leg(float x, float z, const std::shared_ptr<ObjPLY>& leg_mesh);
    static std::shared_ptr<CompositionNode> build_tail(const std::shared_ptr<ObjPLY>& tail_1_mesh, const std::shared_ptr<ObjPLY>& tail_2_mesh);
    static std::shared_ptr<CompositionNode> build_ear(const std::shared_ptr<ObjPLY>& ear_mesh);
    static std::shared_ptr<CompositionNode> build_body(float y, const std::shared_ptr<ObjPLY>& body_mesh);
public:
    Chipmunk();
