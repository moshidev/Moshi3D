...
void Mesh3D::make_current_buffer_data_list(void) {
    ...
    list.splice(list.begin(), mklist_chess_mode(get_vertices_VB(), indices_IB));
}
...
std::list<Mesh3D::BufferedData> Mesh3D::mklist_chess_mode(VertexBuffer& vb, IndexBuffer& ib) {
    ...
    if (chess_enabled) {
        ...
        l.emplace_back(vb, ib, color_chess_a_VB, GL_FILL);
        l.back().set_face_indices_offset(0, count);
        l.emplace_back(vb, ib, color_chess_b_VB, GL_FILL);
