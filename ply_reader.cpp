...
)
{
    ...
    cout << "archivo ply '" << na
         << "' leido: núm. vértices == " << num_vertices
         << ", núm caras == " << num_caras << endl
         << flush;
}

//**********************************************************************

void read_vertices(const std::string &nombre_archivo_pse,
                ...
                vector<Tupla3i> &caras, ifstream &src)
{
    ...
    for (long ifa = 0; ifa < num_caras; ifa++) {
        ...
        if (nv != 3)
            error("encontrada una cara con un número de vértices distinto del "
                  "prefijado 3");
        ...
        for (unsigned ivc = 0; ivc < 3; ivc++) {
            ...
            if (num_vertices <= iv[ivc])
                error("encontrado algún índice de vértice igual o superior al "
                      "número de vértices");
        }
        src.getline(buffer, tam_buffer); // ignore more properties, so far ...
                   ...
                   const bool lee_num_caras)
{
    char buffer[unsigned(tam_buffer)];
    string token;
    unsigned state = 0; // 0 antes de leer 'element vertex' (o 'element face'),
                        // 1 antes de leer 'element face', 2 después
    ...
    while (en_cabecera) {
        ...
        if (token == "end_header") {
            if (state != 2)
                error("no encuentro 'element vertex' o 'element face' en la "
                      "cabecera");
            ...
            src >> token;
            if (token != "ascii") {
                string msg = string("el formato del ply no es 'ascii' es '") +
                             token + "', no lo puedo leer";
                error(msg.c_str());
            }
            src.getline(buffer, tam_buffer);
        }
            ...
            if (token == "vertex") {
                if (state != 0)
                    error("la línea 'element vertex' va después de 'element "
                          "face'");
                src >> nv;
            ...
            else { // cout << "  elemento '" + token + "' ignorado." << endl ;
            }
            src.getline(buffer, tam_buffer);
        }
    ...
    if (nv <= 0)
        error("no se ha encontrado el número de vértices, o bien es 0 o "
              "negativo");

    if (lee_num_caras)
        if (nc <= 0)
            error("no se ha encontrado el número de caras, o bien es 0 o "
                  "negativo");

    if (nv > numeric_limits<int>::max())
        error("el número de vértices es superior al valor 'int' más grande "
              "posible.");

    if (lee_num_caras)
        if (nc > numeric_limits<int>::max())
            error("el número de caras es superior al valor 'int' más grande "
                  "posible.");
...
void abrir_archivo(std::string &nombre_archivo, ifstream &src)
{
    ...
    if (!src.is_open()) {
        string msg = string("no puedo abrir el archivo '") + nombre_archivo +
                     "' para lectura.";
        error(msg.c_str());
    }
